    ...
    def prepare(
        ...
        *,
        ...
        if spec is None or not spec.enabled:
            raise ConnectorRegistryError("connector is not registered or is disabled")
        if self._registry.resolve_tool(connector_id, tool_name) is None:
            raise ConnectorRegistryError("tool is not exposed by this connector or is unavailable")
        _validate_input_schema(input_data, spec.input_schema or {})
        ...
        return ConnectorPreparation(ConnectorRequest(connector_id, tool_name, input_data, credential_reference), decision)
