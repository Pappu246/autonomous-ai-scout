...
_CAPABILITY_TO_OPERATION={Capability.INSPECT:"inspect",Capability.TEST:"test",Capability.LINT:"lint",Capability.METRICS:"metrics",Capability.READ_FILE:"read_file",Capability.BENCHMARK:"benchmark",Capability.WEB_RESEARCH:"web_research",Capability.FILES_WORKSPACE:"filesystem_workspace"}
MAX_RETRIES=2
...
def execute_plan(plan:TaskPlan,root:Path,*,granted:Iterable[Capability|str]=(),explicitly_approved=False,sandbox_available=True,audit_path:Path,execution_id:str,registry:ToolRegistry=REGISTRY,max_retries=0,timeout_seconds=30,output_limit=MAX_OUTPUT_BYTES,memory:CrossProjectMemory|None=None,project="local",web_connector:Any=None,web_request:Mapping[str,Any]|None=None,workspace_connector:Any=None,workspace_request:Mapping[str,Any]|None=None)->ExecutionResult:
    ...
    for step in plan.steps:
        ...
        if tool.read_write_mode.value!="read_only" and not (capability is Capability.FILES_WORKSPACE and explicitly_approved):return ExecutionResult(ExecutionState.BLOCKED,"write operation requires explicit approval",total_attempts,tuple(results),str(audit_path))
        if not tool.safe_autonomous and not (capability is Capability.FILES_WORKSPACE and explicitly_approved):return ExecutionResult(ExecutionState.BLOCKED,f"tool is outside the safe autonomous execution boundary: {tool.name}",total_attempts,tuple(results),str(audit_path))
