...
def _validate_documents_tool(tool):
...
def execute_plan(plan:TaskPlan,root:Path,*,granted:Iterable[Capability|str]=(),explicitly_approved=False,origin_trust:TrustLevel=TrustLevel.USER,sandbox_available=True,audit_path:Path,execution_id:str,registry:ToolRegistry=REGISTRY,max_retries=0,timeout_seconds=30,output_limit=MAX_OUTPUT_BYTES,memory:CrossProjectMemory|None=None,project="local",web_connector:Any=None,web_request:Mapping[str,Any]|None=None,workspace_connector:Any=None,workspace_request:Mapping[str,Any]|None=None,gmail_connector:Any=None,gmail_request:Mapping[str,Any]|None=None,calendar_connector:Any=None,calendar_request:Mapping[str,Any]|None=None,browser_connector:Any=None,browser_request:Mapping[str,Any]|None=None,documents_connector:Any=None,documents_request:Mapping[str,Any]|None=None,application_connector:Any=None,application_request:Mapping[str,Any]|None=None,checkpoint_path:Path|None=None)->ExecutionResult:
    ...
    for step in plan.steps:
        ...
        if capability is Capability.DOCUMENTS and not _validate_documents_tool(tool):return ExecutionResult(ExecutionState.BLOCKED,"documents tool is incompatible with the safe sandbox boundary",total_attempts,tuple(results),str(audit_path))
        if capability is Capability.APPLICATION and not _validate_application_tool(tool):return ExecutionResult(ExecutionState.BLOCKED,"application tool is incompatible with the safe sandbox boundary",total_attempts,tuple(results),str(audit_path))
        ...
        for attempt in range(retries+1):
            ...
            if capability is Capability.BROWSER and isinstance(request,dict):request["operation"]={"browser.open":"open","browser.click":"click","browser.extract":"extract"}.get(tool.name,request.get("operation",""))
            if capability is Capability.DOCUMENTS and isinstance(documents_request,Mapping):candidate=documents_request.get(tool.name,documents_request);request=dict(candidate) if isinstance(candidate,Mapping) else None;connector=documents_connector
            if capability is Capability.DOCUMENTS and isinstance(request,dict) and "operation" not in request:request["operation"]={"documents.inspect":"inspect","documents.extract.text":"extract_text","documents.extract.tables":"extract_tables","documents.page.read":"read_page","documents.transform":"transform"}.get(tool.name,tool.name)
            if capability is Capability.APPLICATION and isinstance(application_request,Mapping):candidate=application_request.get(tool.name,application_request);request=dict(candidate) if isinstance(candidate,Mapping) else None;connector=application_connector
            if capability is Capability.APPLICATION and isinstance(request,dict) and "operation" not in request:request["operation"]={"application.list":"list_applications","application.inspect":"get_application","application.observe":"observe","application.command.execute":"execute_command"}.get(tool.name,tool.name)
            result=run_safe_operation(operation,root,timeout_seconds=timeout,output_limit=output,web_connector=connector if capability is Capability.WEB_RESEARCH else None,web_request=request if capability is Capability.WEB_RESEARCH else None,workspace_connector=connector if capability in {Capability.READ_FILE,Capability.FILES_WORKSPACE,Capability.WORKSPACE_SHELL} else None,workspace_request=request if capability in {Capability.READ_FILE,Capability.FILES_WORKSPACE,Capability.WORKSPACE_SHELL} else None,gmail_connector=connector if capability is Capability.EMAIL else None,gmail_request=request if capability is Capability.EMAIL else None,calendar_connector=connector if capability is Capability.CALENDAR else None,calendar_request=request if capability is Capability.CALENDAR else None,browser_connector=connector if capability is Capability.BROWSER else None,browser_request=request if capability is Capability.BROWSER else None,documents_connector=connector if capability is Capability.DOCUMENTS else None,documents_request=request if capability is Capability.DOCUMENTS else None,application_connector=connector if capability is Capability.APPLICATION else None,application_request=request if capability is Capability.APPLICATION else None);results.append(result);_audit(audit_path,execution_id,ExecutionState.RUNNING,tool=tool.name,step_id=step.step_id,attempt=attempt+1,result="success" if result.success else "failure",verification=result.verification_status,event="tool_result")
