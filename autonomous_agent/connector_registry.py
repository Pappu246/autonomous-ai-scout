...
def _validate(spec,tools):
    ...
    for name in spec.registered_tools:
        ...
        if tool.network_requirement.value!=spec.network_requirement.value and spec.category!="web":raise ConnectorRegistryError("connector network contract mismatch")
...
def web_connector(tool_registry=REGISTRY):
    ...
    network=ConnectorSpec("web_research_network","Bounded public web network reads","web",(Capability.WEB_RESEARCH.value,), ("public:read",),ConnectorAuth.NONE,CredentialHandling.NONE,NetworkRequirement.REQUIRED,ReadWriteMode.READ_ONLY,RiskLevel.MEDIUM,ApprovalRequirement.NONE,SandboxRequirement.REQUIRED,AuditRequirement.REQUIRED,("web.search","web.read"),True,input_schema=schema,output_schema=schema)
    local=ConnectorSpec("web_research_local","Bounded deterministic web evidence processing","web",(Capability.WEB_RESEARCH.value,), ("public:evidence",),ConnectorAuth.NONE,CredentialHandling.NONE,NetworkRequirement.NONE,ReadWriteMode.READ_ONLY,RiskLevel.LOW,ApprovalRequirement.NONE,SandboxRequirement.REQUIRED,AuditRequirement.REQUIRED,("web.extract","web.compare"),True,input_schema=schema,output_schema=schema)
    return ConnectorRegistry((network,local),tool_registry=tool_registry)
