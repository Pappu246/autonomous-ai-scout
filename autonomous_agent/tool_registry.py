...
_BROWSER_INPUT={"type":"object","properties":{"operation":{"type":"string"},"url":{"type":"string"},"selector":{"type":"string"},"fields":{"type":"array"},"timeout_seconds":{"type":"integer"}},"additionalProperties":False}\n_REST_INPUT={"type":"object","properties":{"method":{"type":"string"},"url":{"type":"string"},"headers":{"type":"object"},"body":{"type":"string"},"credential_ref":{"type":"string"},"idempotency_key":{"type":"string"}},"required":["url"],"additionalProperties":False}
...
_tool("filesystem.write","Write only inside an approved workspace after explicit approval.","files",RiskLevel.HIGH,ReadWriteMode.CONTROLLED_WRITE,NetworkRequirement.NONE,AuthenticationRequirement.NONE,ApprovalRequirement.EXPLICIT,SandboxRequirement.REQUIRED,AuditRequirement.REQUIRED,Capability.FILES_WORKSPACE,safe_autonomous=False,input_schema=_FILES_INPUT),
_tool("filesystem.transform","Transform an approved workspace file after explicit approval.","files",RiskLevel.HIGH,ReadWriteMode.CONTROLLED_WRITE,NetworkRequirement.NONE,AuthenticationRequirement.NONE,ApprovalRequirement.EXPLICIT,SandboxRequirement.REQUIRED,AuditRequirement.REQUIRED,Capability.FILES_WORKSPACE,safe_autonomous=False,input_schema=_FILES_INPUT),
_tool("workspace.shell","Run a bounded read-only workspace shell command.","workspace",RiskLevel.LOW,ReadWriteMode.READ_ONLY,NetworkRequirement.NONE,AuthenticationRequirement.NONE,ApprovalRequirement.NONE,SandboxRequirement.REQUIRED,AuditRequirement.REQUIRED,Capability.WORKSPACE_SHELL,safe_autonomous=True,input_schema={"type":"object","properties":{"argv":{"type":"array"}},"required":["argv"],"additionalProperties":False}),
...
_tool("browser.open","Open a bounded HTTPS page through the controlled browser transport.","browser",RiskLevel.MEDIUM,ReadWriteMode.READ_ONLY,NetworkRequirement.REQUIRED,AuthenticationRequirement.NONE,ApprovalRequirement.NONE,SandboxRequirement.REQUIRED,AuditRequirement.REQUIRED,Capability.BROWSER,safe_autonomous=True,input_schema=_BROWSER_INPUT),
_tool("browser.click","Click a bounded selector through the controlled browser transport.","browser",RiskLevel.MEDIUM,ReadWriteMode.READ_ONLY,NetworkRequirement.REQUIRED,AuthenticationRequirement.NONE,ApprovalRequirement.NONE,SandboxRequirement.REQUIRED,AuditRequirement.REQUIRED,Capability.BROWSER,safe_autonomous=True,input_schema=_BROWSER_INPUT),
_tool("browser.extract","Extract bounded text/links through the controlled browser transport.","browser",RiskLevel.LOW,ReadWriteMode.READ_ONLY,NetworkRequirement.REQUIRED,AuthenticationRequirement.NONE,ApprovalRequirement.NONE,SandboxRequirement.REQUIRED,AuditRequirement.REQUIRED,Capability.BROWSER,safe_autonomous=True,input_schema=_BROWSER_INPUT),
 _tool("rest.get","GET an explicitly allowlisted REST endpoint through the bounded generic REST connector.","rest",RiskLevel.MEDIUM,ReadWriteMode.READ_ONLY,NetworkRequirement.REQUIRED,AuthenticationRequirement.USER_AUTH,ApprovalRequirement.NONE,SandboxRequirement.REQUIRED,AuditRequirement.REQUIRED,Capability.REST_API,safe_autonomous=True,input_schema=_REST_INPUT),
 _tool("rest.head","HEAD an explicitly allowlisted REST endpoint through the bounded generic REST connector.","rest",RiskLevel.MEDIUM,ReadWriteMode.READ_ONLY,NetworkRequirement.REQUIRED,AuthenticationRequirement.USER_AUTH,ApprovalRequirement.NONE,SandboxRequirement.REQUIRED,AuditRequirement.REQUIRED,Capability.REST_API,safe_autonomous=True,input_schema=_REST_INPUT),
 _tool("rest.write","Perform an approved REST write through the bounded generic REST connector.","rest",RiskLevel.HIGH,ReadWriteMode.CONTROLLED_WRITE,NetworkRequirement.REQUIRED,AuthenticationRequirement.USER_AUTH,ApprovalRequirement.HUMAN_REVIEW,SandboxRequirement.REQUIRED,AuditRequirement.REQUIRED,Capability.REST_API,safe_autonomous=False,input_schema=_REST_INPUT),
...
_tool("calendar.event.create","Create an event only after human review.","calendar",RiskLevel.HIGH,ReadWriteMode.CONTROLLED_WRITE,NetworkRequirement.REQUIRED,AuthenticationRequirement.USER_AUTH,ApprovalRequirement.HUMAN_REVIEW,SandboxRequirement.REQUIRED,AuditRequirement.REQUIRED,Capability.CALENDAR,safe_autonomous=False,input_schema=_CALENDAR_INPUT),
_tool("calendar.event.update","Update an event only after human review.","calendar",RiskLevel.HIGH,ReadWriteMode.CONTROLLED_WRITE,NetworkRequirement.REQUIRED,AuthenticationRequirement.USER_AUTH,ApprovalRequirement.HUMAN_REVIEW,SandboxRequirement.REQUIRED,AuditRequirement.REQUIRED,Capability.CALENDAR,safe_autonomous=False,input_schema=_CALENDAR_INPUT),
_tool("calendar.event.cancel","Cancel an event only after human review.","calendar",RiskLevel.CRITICAL,ReadWriteMode.HIGH_RISK_WRITE,NetworkRequirement.REQUIRED,AuthenticationRequirement.USER_AUTH,ApprovalRequirement.HUMAN_REVIEW,SandboxRequirement.REQUIRED,AuditRequirement.REQUIRED,Capability.CALENDAR,safe_autonomous=False,input_schema=_CALENDAR_INPUT),
_tool("github.change","Prepare a source change after explicit approval; merge remains separately gated.","github",RiskLevel.HIGH,ReadWriteMode.CONTROLLED_WRITE,NetworkRequirement.REQUIRED,AuthenticationRequirement.USER_AUTH,ApprovalRequirement.EXPLICIT,SandboxRequirement.REQUIRED,AuditRequirement.REQUIRED,Capability.SOURCE_WRITE,safe_autonomous=False),
