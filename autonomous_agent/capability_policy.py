...
def normalize_capability(value):
    ...
    if normalized not in {x.value for x in Capability}:
...
def check_capability(value, granted: Iterable[Capability | str] = ()):
    try:
        ...
        granted_normalized = {normalize_capability(x) for x in granted}
    except (TypeError, ValueError):
        return CapabilityDecision(False, "capability input is invalid", "")
    if capability in {x.value for x in DENIED_CAPABILITIES}:
        return CapabilityDecision(
            False, "capability is permanently denied by autonomous policy", capability
        )
    if capability not in granted_normalized:
        return CapabilityDecision(
            False, "capability has not been explicitly granted", capability
        )
    return CapabilityDecision(
        True, "capability is explicitly granted and within the safe allowlist", capability
    )
