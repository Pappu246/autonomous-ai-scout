...
def inspect_recovery(path: Path, action_id: str) -> RecoveryDecision:
    ...
    return RecoveryDecision(True, "persisted lifecycle state is recoverable", state)


def recover_stale_execution(path: Path, action_id: str) -> RecoveryDecision:
    """Convert an interrupted claimed/executed action into a terminal blocked state.
    ...
    if not decision.recoverable:
        return decision
    if decision.state not in {LifecycleState.CLAIMED, LifecycleState.EXECUTED}:
        return RecoveryDecision(False, "stale recovery is only valid for interrupted execution states", decision.state)
    try:
