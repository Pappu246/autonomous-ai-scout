...
from .execution_audit import verify_execution_audit
...
def run_task(
    ...
    *,
    ...
    if not plan.executable:
        return ExecutionResult(ExecutionState.BLOCKED, plan.reason, 0, (), str(audit_path))
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    result = run_task(args.task, root=Path(args.root).resolve(), audit_path=Path(args.audit).resolve())
