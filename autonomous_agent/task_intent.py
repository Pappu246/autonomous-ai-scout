...
def classify_intent(task:str)->TaskIntent:
    ...
    def positive_contains(*terms: str) -> bool:
        return any(
    ...
    # Positive change requests stay change requests even when they mention tests.
    if positive_contains("fix", "change", "edit", "modify", "update code", "repair"):return TaskIntent.CHANGE
    ...
    # Explicit test requests are recognized unless the positive task is a change.
    if positive_contains("test suite", "run tests", "pytest", "unit tests", "integration tests"):return TaskIntent.TEST
