...
_INTENT_TO_TOOLS = {
    "research": ("web.search", "web.read", "web.extract", "web.compare"),
...
def _digest(task, intent, tool_names):
    return hashlib.sha256(json.dumps({"task": task, "intent": intent, "tools": tool_names}, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
...
def plan_task(task: str, *, granted: Iterable[Capability | str] = (), explicitly_approved=False, sandbox_available=True, audit_available=True, registry: ToolRegistry = REGISTRY):
    raw = " ".join(task.strip().split()); intent = classify_intent(raw); required = _INTENT_TO_TOOLS.get(intent.value, ()); selected = _select_tools(registry, intent.value); descriptions = decompose_task(raw, intent)
    missing = tuple(name for name in required if registry.get(name) is None)
    ...
    for index, tool in enumerate(selected, 1):
        ...
        decision = registry.authorize(tool.name, granted, explicitly_approved=explicitly_approved, sandbox_available=sandbox_available, audit_available=audit_available)
    ...
    digest = _digest(raw, intent.value, tuple(s.tool_name for s in steps))
    return TaskPlan(raw, intent, tuple(steps), aggregate_risk(selected), executable, reason, TaskAuditRecord(raw, intent, tuple(s.step_id for s in steps), executable, digest))
