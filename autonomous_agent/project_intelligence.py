...
def analyze_project(root: Path, repository: str = "local") -> list[ProjectFinding]:
    ...
    assignment_pattern = re.compile(r"(?:api[_-]?key|secret|token|password)\s*[=:]\s*['\"]([^'\"]+)['\"]", re.I)
    for path in root.rglob("*"):
        ...
        if any(_looks_like_secret_assignment(match) for match in assignment_pattern.finditer(text)) or any(pattern.search(text) for pattern in SECRET_PATTERNS[1:]):
            if path.name not in {".env.example", ".env.sample"}:
                findings.append(_finding(repository, "high", "Possible hard-coded secret", f"A high-confidence secret-like assignment was detected in {path.relative_to(root)}.", "Move credentials to environment/secret storage and rotate exposed credentials."))
                break
