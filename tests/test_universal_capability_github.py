...
def test_universal_schema_is_complete_and_deterministic():
    spec=CapabilitySpec("github:inspect",Domain.GITHUB,"github.inspect",{"type":"object"},{"type":"object"},"low","read_only","required","user_auth","github","none","required","required",("repository:read",),IdempotencyMode.NATURAL,RetryPolicy(2,1),True); assert spec.fingerprint==spec.fingerprint and spec.version==1

def test_authenticated_capability_cannot_embed_credential_material():
    with pytest.raises(ValueError): CapabilitySpec("github:bad",Domain.GITHUB,"github.inspect",{"type":"object"},{"type":"object"},"low","read_only","required","user_auth","token=secret","none","required","required",("repository:read",),IdempotencyMode.NATURAL,RetryPolicy(),True)
def test_write_capability_requires_approval():
    with pytest.raises(ValueError): CapabilitySpec("github:write",Domain.GITHUB,"github.change",{"type":"object"},{"type":"object"},"high","controlled_write","required","user_auth","github","none","required","required",("repository:change",),IdempotencyMode.REQUIRED,RetryPolicy(),True)
def test_github_registry_delegates_authority_to_existing_tool_registry():
    registry=github_capabilities(REGISTRY); assert {x.capability_id for x in registry.list(domain=Domain.GITHUB)}=={"github:change","github:inspect","github:status"}; assert registry.authorize("github:inspect",("inspect",)).allowed; assert not registry.authorize("github:change",("source_write",)).allowed
def test_disabled_capability_is_fail_closed():
    registry=CapabilityRegistry(REGISTRY); spec=CapabilitySpec("github:disabled",Domain.GITHUB,"github.inspect",{"type":"object"},{"type":"object"},"low","read_only","required","user_auth","github","none","required","required",("repository:read",),IdempotencyMode.NATURAL,RetryPolicy(),False); registry.register(spec); assert not registry.authorize("github:disabled",("inspect",)).allowed
def fake_fetch(path,params=None):
    if path=="/repos/owner/repo":return {"full_name":"owner/repo","id":1,"default_branch":"main","archived":False,"fork":False}
    if path=="/repos/owner/repo/pulls/7":return {"state":"open","draft":True,"base":{"ref":"main","sha":BASE,"repo":{"full_name":"owner/repo"}},"head":{"ref":"feature/x","sha":SHA,"repo":{"full_name":"owner/repo"}}}
    if path==f"/repos/owner/repo/commits/{SHA}/check-runs":return {"check_runs":[{"name":"CI","status":"completed","conclusion":"success","head_sha":SHA}]}
    raise AssertionError(path)
def test_github_connector_uses_verified_repository_scope_and_read_api():
    c=GitHubDomainConnector(fake_fetch); assert c.repository("owner/repo")["default_branch"]=="main"; pr=c.pull_request("owner/repo",7); assert pr["head_sha"]==SHA and pr["base_sha"]==BASE; assert c.checks("owner/repo",SHA)[0]["conclusion"]=="success"
