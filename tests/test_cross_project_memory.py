...
def test_finding_deduplication_uses_stable_fingerprint(tmp_path: Path):
    memory = CrossProjectMemory(tmp_path / "memory.json")
    memory.record_finding("owner/repo", "missing license", severity="medium")
    ...
    memory.record_finding("owner/repo", "missing license", severity="medium", status="repeated")
    assert len(memory.learn("owner/repo", kind="finding")) == 2
