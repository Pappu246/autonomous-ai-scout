...
def test_symlink_escape_is_blocked(tmp_path):
    outside=tmp_path.parent/"outside-n3.txt";outside.write_text("secret",encoding="utf-8")
    link=tmp_path/"link";link.symlink_to(outside)
    try:
        with pytest.raises(WorkspaceError):c:=_workspace(tmp_path);c.read("link")
...
def test_secret_leakage_is_blocked(tmp_path):
    c=_workspace(tmp_path)
    with pytest.raises(WorkspaceError):c.write("secret.txt","api_key=sk-secret")
def test_write_requires_authorization_and_approval(tmp_path):
    p=plan_task("write file",granted=[Capability.FILES_WORKSPACE]);assert not p.executable
    p=plan_task("write file",granted=[Capability.FILES_WORKSPACE],explicitly_approved=True);assert p.executable
...
def test_capability_mismatch_fails_closed(tmp_path):
    p=plan_task("read file",granted=[Capability.WEB_RESEARCH]);assert not p.executable
...
def test_memory_persistence_failure_is_observational(tmp_path):
    ...
    m=Broken(tmp_path/"m.json");assert not m.record_finding("a",{"kind":"filesystem","fingerprint":"x"})
def test_audit_and_cross_project_evidence_do_not_grant_authority(tmp_path):
