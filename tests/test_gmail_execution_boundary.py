...
class Transport:
    def request(self,method,url,*,params=None,body=None,timeout_seconds=10):
        ...
        if "/messages/m1" in url:return {"id":"m1","threadId":"t1","labelIds":["INBOX"],"snippet":"invoice"}
        if "/threads/t1" in url:return {"id":"t1","messages":[{"id":"m1","threadId":"t1","internalDate":"1","snippet":"invoice"}]}
        raise AssertionError(url)
def test_gmail_read_flows_through_existing_executor(tmp_path: Path):
    registry=ToolRegistry();register_gmail_tools(registry);plan=plan_task("search Gmail for invoices",granted=(Capability.EMAIL,),registry=registry)
    ...
    assert result.state is ExecutionState.VERIFIED and all(x.verification_status=="verified" for x in result.results) and "gmail-read-1" in audit.read_text(encoding="utf-8")
